...
- fix(checkout): batched per-item variant/product lookups in `Confirm`/`Preview` into single `FindByIDs` calls instead of one query per item — eliminates an N+1 pattern on carts with many line items.
- fix(media): upload `scope` param was passed straight into the storage key path with no allowlist check — validated against the known scope set before use.
- fix(grpcclient): payment/notification gRPC clients always dialed in plaintext regardless of environment — TLS is now gated on `app.env`, matching the rest of the codebase's env-aware security posture.
- fix(config): reject known placeholder secrets (`change-me-in-env`, etc.) outside `development`, so a misconfigured production deploy fails fast instead of running with a default secret.
- fix(order): `GET /admin/orders` route existed but had no handler wired — implemented `ListAdmin`.
- refactor(product): dropped `product` usecase's import of `catalog` usecase (inverted layering) for a cache-key helper — inlined the key format instead.
- chore: deleted the dead `usecase/payment` package (superseded by the gRPC `PaymentGateway` client, no remaining references).
- refactor: extracted a shared notify-user helper used by multiple usecases that were each hand-rolling the same notification-insert-plus-email-enqueue sequence.
- refactor: unexported `Container`'s handler/repo fields — external packages only ever need the constructor, not field access.
- refactor: extracted `Summary.BuildCSV`/`AdminSummary.BuildCSV` methods on the report usecase, replacing duplicated inline CSV-building code in the worker's two `report:generate*` Asynq task handlers.
- fix: internal-secret comparison on `POST /internal/payments/callback` switched from `==` to `crypto/subtle.ConstantTimeCompare` (timing side-channel); `/health` response wrapped in the standard `pkg/response` envelope; `ForceStatusRequest.Status` validated against the known order-status set via `oneof`; added indexes on `orders.status` and `order_status_histories(to_status, created_at)` for the hot query paths that filter on them.
- fix: replaced golang-migrate's `lib/pq`-backed `database/postgres` driver with its `database/pgx/v5` driver in `cmd/migrate` and the postgres-repository integration test, so the migration tool uses the same pgx driver as the rest of the app; `lib/pq` is now only an indirect transitive dependency. Verified with a fresh disposable Postgres container: `up` → `down` → `up` all clean.
- refactor: replaced `route.Register`'s 27-parameter positional argument list with a `route.Deps` struct, keyed by name at the call site — the prior signature made it easy to pass args in the wrong order with no compiler error (multiple adjacent params share the same handler pointer type).
- `make lint && make test` green after all fixes above; migrations verified clean (`up` → `down` → `up`) against a disposable Postgres container; `docker compose up` stack (postgres, redis, minio, api, worker, payment-service, notification-service) healthy.
